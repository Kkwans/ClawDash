...
async function removeChannel(ch) {
  const ok = await showConfirm(`确定删除渠道 ${getChannelName(ch.id)}？此操作不可恢复。`)
  if (!ok) return
  try {
    const cfgRes = await gwRequest('config.get')
    const raw = getRawConfig(cfgRes)
  ...
  <div class="space-y-6">
            ...
            <div v-for="f in selectedChannelType.fields" :key="f.key">
              ...
              <div class="relative">
                <input v-model="addForm.config[f.key]"
                  :type="f.type === 'password' && !showPassword['add_' + f.key] ? 'password' : 'text'"
                  :placeholder="f.placeholder"
                  class="w-full border rounded-lg px-3 py-2 text-sm pr-10">
                <button v-if="f.type === 'password'"
                  @click="togglePassword('add_' + f.key)"
                  class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                  {{ showPassword['add_' + f.key] ? '🙈' : '👁️' }}
                </button>
              </div>
            </div>
          </template>
    ...
    <div v-if="showInstallModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div class="bg-white rounded-xl p-6 w-full max-w-lg mx-4 shadow-2xl">
        ...
        <!-- 插件仓库 -->
        <div class="mb-4">
          <label class="text-xs text-gray-500 mb-2 block">推荐插件</label>
          <div class="space-y-2">
            <div v-for="plugin in presetPlugins" :key="plugin.id"
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-500">{{ plugin.desc }}</p>
              </div>
              <span class="text-xs text-blue-600">安装</span>
            </div>
          </div>
        </div>
        ...
        <div class="border-t pt-4">
          <label class="text-xs text-gray-500 mb-2 block">手动输入插件 ID</label>
        ...
        <div class="mt-4 text-right">
          <button @click="showInstallModal = false" class="px-4 py-2 text-sm text-gray-600 border rounded-lg hover:bg-gray-50">关闭</button>
        </div>
      </div>
    </div>

    <!-- 渠道详情弹窗 -->
    <div v-if="showChannelDetail" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div class="bg-white rounded-xl p-6 w-full max-w-md mx-4 shadow-2xl">
        <div class="flex items-center gap-3 mb-4">
          <span class="text-2xl">{{ channelIcons[showChannelDetail.id] || '📡' }}</span>
          <div>
            <h3 class="text-lg font-bold">{{ getChannelName(showChannelDetail.id) }}</h3>
            <p class="text-xs text-gray-500">{{ showChannelDetail.id }}</p>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg mb-4">
          ...
          <span class="text-sm font-medium" :class="showChannelDetail.enabled ? 'text-green-600' : 'text-gray-500'">
            ...
            <div v-if="key !== 'enabled'">
              ...
              <div class="relative">
                <input v-model="showChannelDetail.config[key]"
                  :type="(key.toLowerCase().includes('secret') || key.toLowerCase().includes('token')) && !showPassword['detail_' + key] ? 'password' : 'text'"
                  class="w-full border rounded-lg px-3 py-2 text-sm font-mono pr-10">
                <button v-if="key.toLowerCase().includes('secret') || key.toLowerCase().includes('token')"
                  @click="togglePassword('detail_' + key)"
                  class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                  {{ showPassword['detail_' + key] ? '🙈' : '👁️' }}
                </button>
              </div>
            </div>
          </template>
        ...
        <div class="flex gap-2 justify-end">
          <button @click="showChannelDetail = null" class="px-4 py-2 text-sm text-gray-600 border rounded-lg hover:bg-gray-50">取消</button>
          <button @click="saveChannelConfig" :disabled="saving" class="px-4 py-2 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50">
            {{ saving ? '保存中...' : '保存配置' }}
          </button>
        </div>
      </div>
      ...
      <div v-for="ch in channelEntries" :key="ch.id"
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
              <div class="flex items-center gap-2">
                <p class="text-sm font-semibold text-gray-900">{{ getChannelName(ch.id) }}</p>
              ...
              <p class="text-xs text-gray-400 mt-0.5 font-mono">{{ ch.id }}</p>
            </div>
          </div>
          <div class="flex gap-2">
            <button @click="openChannelDetail(ch)"
              class="px-3 py-1.5 rounded-lg text-xs font-medium bg-blue-50 text-blue-600 hover:bg-blue-100 border border-blue-200">
              配置
            </button>
